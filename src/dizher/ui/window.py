...
DEBUG = {   # view -> (tooltip, the stage it needs, its image from that stage's Converter or a running stage's snapshot)
    'Projected': ("What the halftoner aims at: each pixel moved to the nearest mix of its cell's pair (Halftone: project), or the image itself when that is off",
                  'halftone', lambda c: c.projected_target()),
